...
if __name__ == '__main__':
    ...
    for test_image_name in tqdm(submission_data.ImageId.unique()):
        ...
        cu_l = cascaded_union(multipolygon_lakes)
        cu_r = cascaded_union(multipolygon_rivers)

        cu_l = transform(lambda x, y: (np.round(float(x) / x_scale, 10), np.round(float(y) / y_scale, 10)), cu_l)
        cu_r = transform(lambda x, y: (np.round(float(x) / x_scale, 10), np.round(float(y) / y_scale, 10)), cu_r)
    ...
    submission_data.to_csv(os.path.join('..', '..', 'predictions', 'subm{}_cls8.csv'.format(SUBMISSION)), index=False)

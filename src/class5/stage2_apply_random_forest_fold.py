...
def collect_data(image_name_, is_train=True):
    # load M-images, 11bit, Size: 8 x H x W
...
def check_on_train(models_, ss_, train_images_):
    for train_image_ in train_images_:
        ...
        p_res_ = np.zeros(shape=(p_data_.shape[0], ))

        for model_ in models_:
            p_res_ += model_.predict(p_data_)
        p_res_ /= float(len(models_))
        p_res_ = np.rint(p_res_)
...
if __name__ == '__main__':
    ...
    for train_image in TRAIN_IMAGE_NAMES2:
    ...
    #
    kf = KFold(n_splits=5, random_state=20065, shuffle=False).split(data)
    ...
    print 'Train set contains {0} / {1}  positive examples. '.format(np.sum(target), data.shape[0])
    ...
    for i, (train_index, test_index) in enumerate(kf):
        ...
        if os.path.exists('{}-{}.model'.format(model_name, i)) and not IS_TUNING:
            model = pickle.load(open('{}-{}.model'.format(model_name, i), 'rb'))
            ss = pickle.load(open('{}-{}.scale'.format(model_name, i), 'rb'))
        else:
            model = train_model(x_train, y_train, x_test, y_test)

            pickle.dump(model, open('{}-{}.model'.format(model_name, i), 'wb'))
            pickle.dump(ss, open('{}-{}.scale'.format(model_name, i), 'wb'))
            models.append(model)

    del (data, target, kf)

    # ================= Make prediction on train images ==============================
    check_on_train(models, ss, dft.ImageId.unique())
    ...
    for test_image in submission_data.ImageId.unique():
        ...
        prediction_res = np.zeros(shape=(prediction_data.shape[0],))

        for model in models:
            prediction_res += model.predict(prediction_data)
        prediction_res /= float(len(models))
        prediction_res = np.rint(prediction_res)

        print 'Summation: ', np.sum(prediction_res)

        prediction_image = np.reshape(prediction_res, newshape=(h, w)) * MAX_8BIT
        # prediction_image = ndimage.binary_fill_holes(prediction_image)

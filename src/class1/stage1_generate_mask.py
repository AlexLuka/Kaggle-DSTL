...
if __name__ == '__main__':
    ...
    for image_name in reversed(train_wkt.ImageId.unique()):
        image_mask_name = os.path.join(MASK_DIR, '{}-mask.png'.format(image_name))
        if os.path.exists(image_mask_name):
            print 'Mask Already Exist'
            continue

        # read image in. The shape of the image is 8 x W x H. Read only one band
        img_16m = tif.imread(os.path.join('..',
                                          '..',
                                          'data',
                                          'sixteen_band',
                                          '{}_M.tif'.format(image_name)))[SLICE_NUMBER, :, :]
        ...
        roads = shapes[shapes.ClassType == 1]
        ...
        # Scale coefficients for M-images
        ...
        # Scale coefficients for Panchromatic images
        ...
        # prepare the data for parallel calculations
        ...
        # slice mask and run in parallel
        res = pool.map(decolorization, input_data)

        baw_image = np.vstack(res)

        misc.toimage(baw_image).save(image_mask_name)

...
def collect_data(image_name_, is_train=True):
    ...
    if is_train:
        baw_image_ = ndimage.imread('{}/{}-mask.png'.format(MASK_DIR, image_name_), flatten=True)
        t_ = np.around(np.reshape(baw_image_, newshape=(w_ * h_,)) / MAX_8BIT)
        return td_, t_
...
def train_model(x_trn, y_trn, x_cv, y_cv):
    ...
    pos = float(y_trn.shape[0] - np.sum(y_trn)) / np.sum(y_trn)
    print 'Ratio of negative to positive examples: ', pos
...
if __name__ == '__main__':
    ...
    for train_image in TRAIN_IMAGE_NAMES5:

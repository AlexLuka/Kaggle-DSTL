...
if __name__ == '__main__':
    ...
    for test_image_name in tqdm(submission_data.ImageId.unique()):
        ...
        # if test_image_name != '6060_4_1':
        #     continue

        img_mask = ndimage.imread('predicted-masks/{1}/{0}.png'.format(test_image_name, MODEL_ID),
                                  flatten=True)
        ...
        cu_l = unary_union(multipolygon_roads)
        ...
        # Bug fix
        polygons2 = []
        if isinstance(cu_l, MultiPolygon):
            polygons2 = []
            for pp in cu_l.geoms:
                if pp.is_valid:
                    polygons2.append(pp)
                else:
                    polygons2.append(pp.buffer(0))

            cu_l = unary_union(MultiPolygon(polygons2))

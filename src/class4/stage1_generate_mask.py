...
if __name__ == '__main__':
    ...
    for image_name in reversed(train_wkt.ImageId.unique()):
        ...
        if os.path.exists('{}/{}-mask.png'.format(MASK_DIR, image_name)):
            print 'Calculated'
            continue

        # contains self intersecting polygon that cause crash, just ignore it
        # if image_name in ['6090_2_0', '6170_4_1']:
        #     continue

        # read image in. The shape of the image is 8 x W x H. Read only one band
        img_16m = tif.imread('../../data/sixteen_band/{}_{}.tif'.format(image_name, 'M'))[SLICE_NUMBER, :, :]
        img_16rgb = tif.imread('../../data/three_band/{}.tif'.format(image_name))[0, :, :]
    ...
    # EXTRACT SHAPES
        ...
        #
        roads = shapes[shapes.ClassType == 4]
        ...
        for multipolygon in roads.MultipolygonWKT.values:
            ...
            for polygon in polygons.geoms:
                ...
                if polygon_scaled.is_valid:
                    objects_on_that_image.append(polygon_scaled)
                else:
                    objects_on_that_image.append(polygon_scaled.buffer(0))
        ...
        if PLOT_POLYGONS:
            ...
            plt.savefig('{}/{}.png'.format(TRAIN_IMG_DIR, image_name), format='png', dpi=500)

            plt.clf()
            plt.cla()

        print 'Generating black and white image'
        ...
        misc.toimage(baw_image).save('{}/{}-mask.png'.format(MASK_DIR, image_name))
        # misc.toimage(img_16m).save('{}/{}-{}.png'.format(IMG_DIR, name, SLICE_NUMBER))
